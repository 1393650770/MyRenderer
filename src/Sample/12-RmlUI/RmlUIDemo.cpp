...
#include "RmlUI/AllRmlDataModel.h"  // generated by MetaParser from RMLUI_BIND_FIELD annotations
...
void RmlUIDemoApp::OnInitScene()
{
	...
	if (m_hud_model.IsValid())
		{
			// Use code-generated bindings (from RMLUI_BIND_FIELD_AS annotations in the header)
			auto* ctor = m_manager->GetModelConstructor(m_hud_model);
			if (ctor)
				MXRender::UI::RmlUI::Generated::BindRmlUIDemoApp(*ctor, this);
...
void RmlUIDemoApp::OnShutdownScene()
{
	...
	m_manager = nullptr;  // singleton already deleted by Shutdown
}
